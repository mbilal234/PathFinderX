...
for i in range(20):
    ...
    for i in range(2, 100):
        for j in range(1, 5):
            G.add_edge(i, random.randint(i,i+j), distance=random.uniform(1, 15), speed=speedCount)
        ...
        try:
            ...
            for i in range(num_hops):
                try:
                    ...
                    pass
            print(f"The delay from source to destination is: {min_delay:.3f} s")
        except:
            print("Connection fails")

    #Copy code
nx.draw(G,pos)
nx.draw_networkx_edges(G,pos,edgelist=list(zip(shortest_path,shortest_path[1:])),edge_color='r',width=1)
